...
git clone https://github.com/andrylviv/spring-cloud-aws-example.git
...
sudo sed -i -e "s/log_group_name = \/var\/log\/messages/log_group_name = \/profile-service/" /etc/awslogs/awslogs.conf
